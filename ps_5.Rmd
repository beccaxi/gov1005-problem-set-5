...
# rid of the pesky warning I'd been getting that I've introduced NA's by
# coercion, since now I'm able to select for only the integer value that I want.
...
# Here I create a function, draw_samples(), that draws 5000 samples of 25
# students each from the university population. I use set.seed(02139) to
# standardize the otherwise randomly drawn distributions to match the solutions
# key. The result of draw_samples() is a tibble containing an indexing column, a
# column "freshmen_sum" that records the number of freshmen drawn in each
# sample, and a column "perc_freshmen" that records the freshmen drawn in each
# sample as a percentage of the whole sample. I also add a column sample_id
# which assigns to each sample an integer value from 1 to 5000, consecutively.

set.seed(02139)
draw_samples <- function() {
  university %>%
    rep_sample_n(size = 25, reps = 5000) %>%
    summarize(freshmen_sum = sum(grade == "freshman")) %>%
    mutate(perc_freshmen = (sum(grade == "freshman")) / 25) %>% 
    mutate(sample_id = 1:5000)
}

# Here, I essentially just save the tibble result of the function draw_samples()
# in an actual tibble named freshmen_dist

freshmen_dist <- draw_samples()

# Finally, I create a plot of the distribution of percent freshmen using
# geom_density, to avoid the unnecessary hassle of having to figure out an
...
plot_dist <- freshmen_dist %>%
  ggplot(aes(x = sample_id)) + 
    geom_histogram() +
    labs(
      title = "Percentage of Freshmen Sampled over 5000 Samples",
      caption = "Sampling 25 university students from population of 6120",
      x = "Samples",
      y = "Percent Freshmen"
    ) 
plot_dist
...
#First, create a list of four different sizes (25,50,100,500). Use sizes <-
#set_names(sizes, names = sizes), to make sizes a “named” list, which will help
#R cycle through your code.

#Reset the seed to 02139. Use map_df to call rep_sample_n to sample from your
#university tibble, using size as an input. Your output should be a tibble with
#3,375,000 rows and four columns; one for the draw number (1:5000), one for the
#size (25,50,100, or 500), the student’s ID, and their grade. If you’re stuck on
#how to cycle through the elements of the “sizes” list, try using size =. in
#your map_df() expression, and then telling R what the .id stands for outside
#the parentheses. You don’t need to print this tibble

#Use a summarize function to count the number of freshmen in each sample, and
#then take the proportion. Your tibble should now have 20,000 rows; one for each
#draw.

#Using geom_density, plot the distribution of the proportion of freshmen by
#sample size.
