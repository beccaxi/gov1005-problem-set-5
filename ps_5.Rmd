...
# Libs 1, and filter out all potential responses with NA values in the four gun
# policy questions. I also select household_gun_owner to set up the next step.
...
# randomly drawn distributions to match the solutions key. My new tibble
# contains an indexing column, a column "freshmen_sum" that records the number
# of freshmen drawn in each sample, and a column "perc_freshmen" that records
# the freshmen drawn in each sample as a percentage of the whole sample. For
# freshmen_sum, I count the number of rows where grade is "freshmen" by using
# length(which()): the which() function returns a vector of the indices where
# the condition is met, the length of which is the count I want. I also add a
# column sample_id which assigns to each sample an integer value from 1 to 5000,
# consecutively. CHANGE FIX THIS
...
# Zoom, I set an appropriate binwidth of 0.01 and add labels.
...
# questions, using size s an input. My output is a tibble with 3,375,000 rows
...
# by sample size. Size determines the fill color and . I apply appropriate
# labels and breaks for fill color based on size. Finally, I apply
# theme_classic() to mimic the given example in the problem set instructions.
...
samples_freshmen_plot <- samples_freshmen %>% 
  ggplot(aes(x = prop_freshmen, fill = size)) +
    ...
    labs(
      ...
      subtitle = "Average sampled percentage gets closer to true percentage as 
