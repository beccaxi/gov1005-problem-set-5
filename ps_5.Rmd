...
# in the household owns a gun. I also include those respondents with values of
# 999, since respondents who "aren't sure" whether anyone in the household owns
# a gun are assumed to not own guns (anywhere in the household). I want to
# disregard respondents who weren't asked (888), weren't sure (999), or skipped
# the question (.). After filtering for those respondents, I take the mean.

r3_1 <- survey %>% 
  select(household_gun_owner, statements_gun_rights) %>% 
  filter(
    household_gun_owner == 3 | household_gun_owner == 999,
...
# 100 to get a percentage value. To get rid of the warning that I've introduced
# NA's by coercion, 
...
university <- tibble(student_id = 1:6120,
                     grade = c(
                       rep("freshman", 1800), 
                       rep("sophomore", 1450),
                       rep("junior", 1570), 
...
# bar-chart-in-ggplot2). Finally, I use theme_classic() to mimic the example
# plot provided in the problem set instructions.
